...
const generateCssAndHtml = async (overlayType: string): Promise<void> => {
  const isBrowser = overlayType === 'browser';
  const targetFile = path.normalize(`./src/${isBrowser ? '' : 'cookie-'}overlay.ts`);
...
(async (): Promise<void> => {
  await ['cookie', 'browser'].forEach((x) => {
    generateCssAndHtml(x).catch((e) => {
      console.error(e);
      process.exit(1);
    });
  });
